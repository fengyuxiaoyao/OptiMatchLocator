...
class HomographyDataset(BaseDataset):
    default_conf = {
        ...
        "mask_dir": "mask/",  # the subdirectory with the masks (optional)
        "image_list": "revisitop1m.txt",  # optional: list or filename of list
        "glob": ["*.jpg", "*.png", "*.jpeg", "*.JPG", "*.PNG", "*.tif"],
        # splits
        "train_size": 100,
        "val_size": 10,
...
class _Dataset(torch.utils.data.Dataset):
    def __init__(self, conf, image_names, split):
        ...
        self.mask_dir = DATA_PATH / conf.data_dir / conf.mask_dir
    ...
    def getitem(self, idx):
        ...
        mask = read_image(self.mask_dir / name, False)
        if img is None:
        ...
        if self.conf.triplet:
            ...
            data2 = self._read_view(mask, self.conf.homography, ps, left=False)
